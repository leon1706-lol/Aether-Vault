...
- `server` / `webui` - slim, single-role images (no Node in `server`, no Python in
  ...
  the OLD `aether-vault-server`/`aether-vault-webui` alias names STOPPED at v1.3.0 (see
  `VERSIONING.md`'s "Removed in v1.3.0" entry) - they now publish as `server-*`/`webui-*`
