...
- `python/av_cli/`: the `av` command surface and every local concern, split so no file is a monolith:
  - `main.py` — thin compat shell: `_AuthRetryGroup` (`core.py`) construction, per-module lazy command registration (`_LAZY_LOADERS`, V1.5.0 — a command module imports only on first resolution of one of its names; `--help` order is `cli.list_commands()`'s sort, unaffected), the PEP 562 lazy `VaultClient`/`BENCHMARK_NAMES`/`_update_readme_test_badge`, and the two monkeypatch-target owners (`_find_source_root`, `_update_readme_test_badge`). `--version` reads `fsutil.get_version()` directly (not `ui.py`, which would drag in `rich`/`questionary` for a plain version string — Probleme.md #150).
  - `launcher.py` (V1.5.0) — the real console-script entry point; `os`/`sys` only at module scope, decides daemon-vs-in-process before touching `click` at all, falls back to `main.py:run`.
  - `core.py` — shared multi-consumer helpers: config/root/logging, staging (`_compute_stage_result`/`apply_stage_result`/`stage_one_file`, avignore/gitignore/default-ignores), CAS restore machinery (`materialize_file`, `_materialize_tree`, `_collect_dirty_paths`), pending-push trio, `upload_commit_objects` (V1.5.0: optional `only_paths` scoping), `_finalize_commit`, meta/hash helpers, thread-count resolution (`resolve_threads`/`configure_native_threads`, V1.5.0). `_AuthRetryGroup.invoke()` wraps every single command dispatch (the 401-retry flow) — its `.client`/`ui` imports are deferred into the except handler and gated on a `sys.modules` peek rather than an eager or naive `isinstance` import, since this method runs on literally every invocation including `--version`/`--help` (Probleme.md #150: this was a ~1.3-1.4s regression hiding behind the P2 lazy-import work, only visible once a command actually *runs*, not just imports).
  - `daemon_protocol.py`/`daemon_common.py`/`daemon_client.py`/`daemon.py`/`cmd_daemon.py` (V1.5.0) — the opt-in background daemon; see the Daemon Contract below.
...
- `src/`: the C++17 performance core — `core.cpp` (safetensors split + CDC chunker + parallel hashing, one shared lazily-sized `ThreadPool` since V1.5.0 — `set_max_threads()`/`AV_THREADS` — instead of one spawned per call), `sha256.cpp` (V1.5.0: bulk `update()`, no longer byte-at-a-time), `thread_pool.h`; bound as the `aether_core` pybind11 extension. Scalar-returning bindings (`hash_file`, `hash_bytes`, `hash_and_copy`, ...) release the GIL (`py::call_guard<gil_scoped_release>`); the two `py::list`-returning ones (`split_and_hash_safetensors`, `chunk_and_hash_file`) instead split into a pure-C++ `*_core()` computed under an explicit `gil_scoped_release`, with the Python container built afterward with the GIL held — releasing the GIL directly around a function that constructs Python objects is undefined behavior, not merely wrong output.
...
Staging turns arbitrary working-tree files into content-addressed objects plus index entries. The whole decision chain lives in `python/av_cli/core.py::_compute_stage_result()` — pure per-file work (hashing, split/chunk, CAS write), touching neither the shared `Index` nor stdout, which is what makes it safe to run on a worker thread. `apply_stage_result()` is the other half: the only part that mutates `Index`/prints, always run on the main thread, one result at a time. `stage_one_file()` (used verbatim by `av stash push`, so shelving never reimplements staging logic) is a thin sequential wrapper gluing the two together — the entry point every caller outside `av add` itself still uses unchanged.

`av add .` (V1.5.0) computes results for multiple files concurrently via a `ThreadPoolExecutor` sized by `--threads`/`AV_THREADS`/`.av/config`'s `"threads"` key (default: auto, sized to CPU count), then applies them serially in the exact order the (sorted) working-tree walk produced — so the resulting index is byte-identical regardless of thread count or which worker finished first. `AV_THREADS=1` (or fewer than 2 files) takes the original single-threaded code path outright, not just a pool of one, so a threading-suspected bug can be isolated by comparison. Every CAS write (`_atomic_publish_object()`) goes to a temp file in the same shard directory first, then `os.replace` — necessary now that two workers can genuinely race the same content-addressed destination (two files with identical bytes), and a real (if smaller) hardening even single-threaded, since it also closes a `Ctrl-C`-mid-copy torn-object window that predates threading entirely.
...
1. **Layer split** — `.safetensors` files are staged via `aether_core.stage_safetensors` (V1.6.0, default on — `AV_STAGE_FUSED=0` reverts to the legacy path below): one sequential pass parses the header, hashes each tensor layer (plus an `__header__` pseudo-layer holding the header bytes) and the whole file, and publishes each layer directly to `.av/objects`, instead of `split_and_hash_safetensors`'s fully-parallel-but-still-per-layer-re-reading hash pass plus a further Python-side per-layer read to slice+write each one. Layer boundaries come straight from the header's declared `data_offsets` (no rolling-hash decision needed, unlike CDC below) — bytes in any gap between declared layers (alignment padding some writers insert) fold into the whole-file hash only, never into a layer's own hash or its own object. A layer larger than `AV_STAGE_BUFFER_MB` (default 32 MiB) streams straight to a temp file instead of buffering the whole layer in memory, since its final content-addressed path can't be chosen until its hash is complete. The one case the fused single pass structurally cannot represent is *overlapping* declared layer ranges — a real possibility against an adversarial/corrupt header, not just a hypothetical (see `split_and_hash_safetensors_core`'s own bounds-checking comment on why the header is treated as attacker-controllable) — so it throws and the caller falls back to the legacy fully-parallel per-layer re-read, which handles overlap for free (each layer independently re-reads its own range from its own file handle). Both paths are proven byte-identical per layer (`tests/test_core.py`), not just independently "close enough."
2. **CDC chunking** — files matching the `CHUNKABLE_EXTS` set are staged via `aether_core.stage_cdc` (V1.6.0, default on — `AV_STAGE_FUSED=0` reverts to the legacy path below): one sequential pass over the file computes the whole-file hash, cuts and hashes each chunk, and publishes each chunk directly to `.av/objects`, instead of three separate read passes (`chunk_and_hash_file`'s own two, plus a further Python-side read to slice+write each chunk). `stage_cdc` shares the exact same cut-decision code (`CdcCutter`, `src/core.cpp`) as the legacy `chunk_and_hash_file` it replaces — the two are proven boundary-identical (`tests/test_core.py`), not just independently "close enough" — and any exception falls back to the legacy two-pass implementation for that file rather than failing the stage. Boundaries: gear-hash rolling cut points with min 512 KB, avg 2 MB, max 8 MB; the max is a soft cap of `max + min − 1` because a cut is only taken when at least `min_chunk` bytes remain after it — the tail never becomes a sliver. `CHUNKABLE_EXTS`: `.pt .pth .ckpt .npz .h5 .hdf5 .pb .msgpack .bin .onnx .model .arrow .feather .pkl .pickle` — uncompressed/block-structured formats where a local edit only shifts nearby chunks. Compressed/columnar containers (`.parquet`, `.zip`/`.gz`/`.tar`/`.7z`) are deliberately excluded by default — an edit there usually rewrites the whole compressed stream, so CDC boundaries wouldn't survive it — but are reachable per-glob via the `chunk` `.avattributes` flag when a repo owner has verified their export path is safe.
...
Directory staging via `av add .` skips anything matching `.avignore` (gitignore-*lite*, one plain glob per line, no negation/anchoring; scaffolded by `av file --avignore`), a real (if partial: no cross-directory `**`, root `.gitignore` only) reading of `.gitignore` when one is present, and (V1.5.0) a built-in default-ignore list of common heavy directories — `venv`, `.venv`, `node_modules`, `build`, `dist`, `.tox`, `.mypy_cache`, `.pytest_cache`, `.eggs`, `site-packages`, and friends — opt-out via `AV_NO_DEFAULT_IGNORES=1`. The old `_IGNORED_DIRS` set (`.av`/`.git`/`__pycache__`) is unconditional and unrelated to any of these three, all layered in `core.py::iter_working_files()`. Already-existing `.av-pointer` files are never re-staged as content. Every staging decision is observable: `av status` reports staged, modified, deleted, and untracked paths from the same index entries `stage_one_file()` writes. When pointers drift out of sync with their objects — interrupted runs, manual deletes — `av doctor --fix` re-links orphaned `.av-pointer` files, downloading from the registry when the object exists only there, and anything unrecoverable surfaces as a `[WARN]` rather than being fabricated.
...
**Stop (V1.5.0, closed):** the no-op `av status`/`av add` gap documented in earlier BENCHMARKS.md captures was root-caused, not just described — `main.py` eagerly imported all ~45 command modules (one of them, transitively, `requests`) on every invocation regardless of which command ran, and `_IGNORED_DIRS` never included `venv`/`node_modules`, so a repo containing either got walked in full on every `add`/`status`. Fixed via lazy per-command-module loading (`main.py`'s `_LAZY_LOADERS`, see `core.py::_AuthRetryGroup`) and the default-ignore list above; current numbers in `development/BENCHMARKS.md`.
...
`upload_commit_objects()` takes an optional `only_paths` set (V1.5.0) scoping its scan to just the tree entries that changed in *this* commit, instead of every tracked file — an unchanged file's object was already confirmed present server-side when it was staged. `commit_staged()` (the everyday `av commit` path) captures its own staged-paths set before `idx.clear_staged()` wipes it and passes it through; `av merge`'s call into `_finalize_commit()` deliberately passes nothing (`None` = full-tree scan), since by the time it gets there `_materialize_tree()` has already cleared every entry's staged flag on that `idx` and there is no cheap way to recover "what did this merge actually change" from it — the full scan there is a correctness floor, not an oversight. `VaultClient.server_available()` also caches its result for 1 second per `VaultClient` instance (V1.5.0) — `_finalize_commit()` calls it twice in quick succession (once via `flush_pending_push()`, once for its own push decision), and this collapses that to one real HTTP round trip without changing behavior for any other caller, none of which call it more than once per command in practice.
...
## Daemon Contract (V1.5.0, default-on since V1.6.0)

`av daemon` is a background process that executes `add`/`status`/`commit`/`push`/`fetch`/`unstage`/`log`/`diff`/`context`/`run` — chosen because they run often enough for Python's own interpreter/import startup (~450-600ms even after the V1.5.0 import-graph fix) to matter, and because every module backing one of them is statically verified (AST-checked, not just trusted) to never prompt — without paying that cost per invocation. It is additive, not a second implementation: `python/av_cli/daemon.py::DaemonServer._execute()` calls `cli.main(args=argv, standalone_mode=False, ...)`, the exact same click command objects the in-process path uses, with `sys.stdout`/`stderr` redirected into buffers and `os.chdir`/an allowlisted env overlay applied for the request's duration. This is what makes a byte-identical-output test against the in-process path possible, and it means the daemon can only ever make a command faster, never behave differently.

**Entry point restructure.** `[project.scripts] av` points at `python/av_cli/launcher.py:main`, not `av_cli.main:run` — `launcher.py` imports only `os`/`sys` at module scope and decides daemon-vs-fallback before touching `click`/`core.py` at all; any fallback (including any exception on the daemon path) does `from .main import run; run()`. `av_cli.main:run` is untouched and still directly invocable, now also under the explicit alias `av-py` (`pyproject.toml`). A separate, Windows-only native launcher (`src/launcher/av_launcher.cpp`) speaks the same wire protocol directly with zero Python startup cost — see its own paragraph below.

**Allowlist gate, one shared implementation.** Which subcommand an argv is actually invoking (skipping `cli`'s own global options like `--output <value>`/`--verbose`) is computed once, in `daemon_common.first_subcommand()`, and used identically by `launcher.py`'s entry gate, `daemon_client.call_daemon()`'s pre-check, and `daemon.py`'s own server-side `handle_request()` re-check. V1.6.0 found and fixed a real bug here (Probleme.md #161): the latter two used to each have their own separate, cruder positional check, silently rejecting `av --output json status` even after `launcher.py`'s own gate had already been fixed for exactly that case.

**Transport.** Never a network-visible port. POSIX: an `AF_UNIX` socket in `$XDG_RUNTIME_DIR/aether-vault/` (falling back to `~/.aether-vault/run/`), directory mode `0700`, socket `0600`. Windows: a named pipe (`\\.\pipe\aether-vault-<user>-<key>`) created with `FILE_FLAG_FIRST_PIPE_INSTANCE` so a same-machine process can't squat the name first; the client opens it as a plain binary file (`open(path, "r+b", buffering=0)`) rather than importing `_winapi`. Framing (`daemon_protocol.py`) is a hand-rolled 4-byte big-endian length prefix plus UTF-8 JSON — chosen over `multiprocessing.connection` specifically because importing that module costs ~337ms client-side (mostly the `_multiprocessing` C extension), more than the entire startup budget the daemon exists to save.

**Naming and version skew.** The socket/pipe name itself is `sha256(protocol_version | cli_version | python_version | av_cli_source_path | realpath(repo_root))[:16]` (`daemon_common.endpoint_key()`) — a daemon started by an older `av` install is simply unreachable to a newer client by construction, no runtime check required. Two more layers catch skew that happens *after* a client has already connected: the handshake compares `cli_version` on every request, and `DaemonServer._version_drifted()` stats `av_cli/__init__.py` and the loaded `aether_core` binary's mtime/size on every request, refusing (`version_skew`) the moment either changes (the realistic `pip install -e .` mid-session case).

**Security.** A 32-byte random token (`secrets.token_hex(32)`) generated per daemon process, written to a `0600` file the client reads before connecting. The request carries the token; the response carries `HMAC(token, client_nonce)` so the client also authenticates the server — defeating a same-machine impersonator that happened to guess/leak the pipe name but not the token. `write_state_file()` writes the token file before the state file, deliberately (see Probleme.md #143): a client only discovers the daemon by first seeing the state file, so by the time that's observable the token must already be final. The env forwarded to the daemon is allowlisted (`AV_*`, `NO_COLOR`, `FORCE_COLOR`, `HOME`/`USERPROFILE`, `COLUMNS`) — never the whole environment.

**Execution model.** Exactly one request executes at a time — a real `threading.Lock` with a 250ms acquire timeout (`busy` response on contention, client falls back in-process; never worse than today). This is what lets the daemon preserve the single-commit-writer invariant around `_finalize_commit()` for free, without any daemon-specific coordination logic.

**State and staleness.** No write-back cache, ever — every command runs against the real filesystem through the same `atomic_write_*` paths as the in-process path; an external `av gc`, `av pull`, or a second `av` process (falling back in-process because the daemon was busy) writing behind the daemon's back is always seen fresh on the next request. There is deliberately no filesystem watcher in V1.5.0 — a missed event would mean `av add` silently skips a modified file, worse than the slowness a watcher would save.

**Lifecycle.** Three-state enablement (`daemon_common.enabled_mode()`), checked in this precedence order: `AV_NO_DAEMON=1` always wins (unconditional escape hatch); `AV_DAEMON` explicitly falsy (`0`/`false`/`no`) or `.av/config`'s `"daemon": {"enabled": false}` forces "use an already-running daemon but never spawn one" for this repo; `AV_DAEMON` explicitly truthy forces auto-spawn regardless of config; otherwise — **the default since V1.6.0** (V1.5.0 defaulted to "use only") — auto-spawn: a cold repo silently fires one off in the background via `daemon_client.maybe_auto_spawn()` (guarded by the same `O_CREAT|O_EXCL` lock file `av daemon start` uses, so two concurrent invocations never double-spawn), while the *current* invocation still always runs in-process rather than waiting on a cold daemon's warm-up. Idle timeout defaults to 900s (`--idle-timeout` overrides); the daemon also exits on `.av` disappearing or 3 consecutive internal errors, so a poisoned daemon cannot serve garbage indefinitely.

**Idle trim (V1.6.1).** A separate, shorter threshold than the idle-timeout shutdown above: `AV_DAEMON_TRIM_SECS` (default 30) of inactivity triggers `DaemonServer.maybe_trim_idle()` on the existing 2s should-stop watchdog tick — `aether_core.release_pool()` (frees the C++ thread pool), `gc.collect()`, and (glibc Linux only; a no-op elsewhere) `ctypes`-called `malloc_trim(0)` to hand freed heap pages back to the OS. Runs at most once per idle stretch (armed again only by the next served request) and never contends with a real command — a non-blocking `_exec_lock` acquire means a trim that can't get the lock this tick just retries on the next one. `av daemon status` surfaces `rss_mb` (Linux: `/proc/self/statm`; Windows: `GetProcessMemoryInfo` via `ctypes`, `wintypes.HANDLE` argtypes required — see Probleme.md #170 for why; macOS: `resource.getrusage`), `uptime_s`, `requests_served`, and `trimmed` (whether at least one trim has happened), all refreshed after every served request and every trim — the state file is a point-in-time snapshot otherwise, previously only written once at startup. V1.6.3: the trim also clears `core._config_cache` (a stat-validated deep copy of every config the daemon ever loaded, repopulated on the next request); `av daemon status` gained `peak_rss_mb` (the kernel high-water mark, via `av_cli/sysres.py`, which now owns every RSS/RAM probe the CLI has); the state-file refresh no longer rewrites the token file unless its content differs; and every request runs with `AV_DAEMON_SERVING=1` in its env overlay, which `av add` uses to `release_pool()` right after its staging batch instead of leaving the C++ pool's idle threads around until the trim fires. rich/questionary/prompt_toolkit stay off the daemon-served paths by test, not by env var (`tests/test_import_graph.py` asserts `status`/no-op `add` never load them) — a flag nobody sets protects nothing.

**Memory envelope (V1.6.3; supersedes the V1.6.1 paragraph).** Every number lives in `development/MEMORY.md` with the before/after capture (`memory-baseline-v1.6.2.json` vs `memory-scoreboard-v1.6.3.json`, both produced by `scripts/rss_scoreboard.py` over the real CLI, measuring the whole process tree — `av` on PATH is a launcher that re-execs the real CLI, so a parent-only probe reports a 4 MB stub). Reference box: daemon idle after trim ≈ 25 MB (self-reported), ≈ 32 MB tree right after serving; `av status` ≈ 31 MB; `commit` 45 MB. The staging peak is now *derived*, not estimated: each worker holds a 1 MiB read buffer plus one layer buffer of at most `AV_STAGE_BUFFER_MB` (32; larger layers stream through a temp file) plus the header held once, so peak ≈ `workers × (AV_STAGE_BUFFER_MB + 2) + ~30 MB` — measured 161 MB at 4 workers, 97 MB with `AV_STAGE_WORKERS_MAX=2`, 33 MB with `AV_STAGE_BUFFER_MB=8` on top, all within 3 MB of the formula; a 1 GiB safetensors costs the same 63 MB as a 256 MiB one. `core.effective_stage_workers()` caps `workers` by `(available RAM − AV_STAGE_RESERVE_MB) // (AV_STAGE_BUFFER_MB + 2)` and by `AV_STAGE_WORKERS_MAX`, so the envelope never exceeds what the box can give. The earlier "≤ 384 MB at 8 workers" figure was a pre-measurement estimate with no derivation and is retired. `tests/test_memory_gate.py` (opt-in, `AV_MEMORY_GATE=1`) and the warn-only `memory-budget` CI job guard the budgets; `av doctor --resources` prints the live picture and recommends the knobs.

**Detached spawning on Windows** tries `CREATE_BREAKAWAY_FROM_JOB` first (pip/distlib's `av.exe` console-script stub commonly wraps its own child in a Job Object with kill-on-close semantics, which a spawned daemon needs to escape), falling back to plain `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP` on `PermissionError` (a job that itself disallows breakaway). See Probleme.md #144 for a documented, verified environment limitation this cannot code around: some sandboxed dev/CI environments tear down *any* detached child (even one launched via Windows Task Scheduler, which normally escapes any job object) on a fixed timeline — `av daemon start --foreground` under an external supervisor is the correct pattern there. (This session's native-launcher verification independently reconfirmed the same constraint: a real spawned daemon on this dev box dies within seconds regardless of what spawns it, which is why `tests/test_launcher_native.py` drives an in-thread `DaemonServer` rather than a real spawn — see that file's own docstring.)

**Native launcher (Windows, V1.6.0).** `src/launcher/av_launcher.cpp` is a separate, small (~600 line) executable that speaks the daemon's real wire protocol (`daemon_protocol.py`, protocol 1 — one request frame, one response frame, no streaming) directly: connect, send, receive, print, exit — with zero Python interpreter startup for an allowlisted command against a warm daemon. It cannot compute `endpoint_key()` itself (that folds in the Python version and this package's own resolved install path), so it uses a separate **discovery-file** handshake instead: it hashes its own resolved exe path + the repo root it found by walking up for a `.av` directory into a 16-hex-char key and looks for `<runtime_dir>/launcher-<key>.json`; on a miss (or a protocol/version mismatch) it falls back to the real Python CLI, setting `AV_LAUNCHER_EXE`/`AV_LAUNCHER_REPO` env vars to the exact two strings it hashed; `daemon_client.call_daemon()`, after any daemon round trip it completes and MAC-verifies, writes that discovery file (hashing those same two env-var strings verbatim, never re-resolving them) if those vars are set. The *next* invocation via the exe finds the file and connects directly. Falls back transparently (never a hang, never an error the user sees) for `--help`, a non-allowlisted command, no repo, `AV_NO_DAEMON`, an unreachable/stale endpoint, or a forged/rejected token — every fallback re-execs the *original, unmodified* argv via `AV_PYTHON` → `av-py.exe` next to itself → `python.exe` next to itself → a bare `python` on `PATH`, in that order. **Not wired to the `av` entry point yet** (ships as `av-native.exe`, a separate opt-in artifact — see `src/launcher/README.md` for the packaging reasoning) and **Windows-only in this revision** (no POSIX toolchain available to build or execute anything for Linux/macOS — same one-oracle-per-claim discipline as the SHA-NI backend). Building its own test suite (`tests/test_launcher_native.py`, a real compiled exe against a real in-thread daemon) found two real, previously-latent bugs — see Probleme.md #161 and the allowlist-gate paragraph above.

| Situation | Behavior |
|---|---|
| No daemon running, mode = use-only (`AV_DAEMON=0` or `.av/config` opt-out) | In-process, unchanged |
| No daemon running, mode = auto-spawn (**the default since V1.6.0**) | In-process for this call; a daemon is fired off in the background for next time |
| Daemon running, command allowlisted | Daemon serves it; output/exit code identical to in-process |
| Daemon running, command not allowlisted (`--help`, `login`, the REPL, ...) | In-process, daemon never contacted |
| Daemon busy (another request mid-flight) | In-process fallback, not a queued wait |
| Any connect/protocol/auth/version-skew failure | In-process fallback, silent unless `AV_DAEMON_DEBUG=1` |
| Native launcher (Windows): discovery file missing/stale/mismatched | Falls back to the real Python CLI, original argv unchanged |
| Native launcher (Windows): discovery file valid, daemon reachable | Direct round trip, zero Python interpreter startup |

## Remote Sync Contract

**Data-plane streaming invariants (V1.6.3).** No object is ever held whole in RAM on either side: the client uploads with an open file handle and downloads through `VaultClient.download_object` (8 MiB `iter_content`, hash verified as it lands, temp-file + rename) — `av registry export/restore` now go through the same path instead of `resp.content`/`read_bytes()`; the server streams uploads to a temp file in 4 MiB pieces (`storage.store_object`, optionally capped by `AV_MAX_UPLOAD_BYTES` — enforced against Content-Length *and* while streaming) and serves downloads with `FileResponse`. `av clone` consumes `sync.iter_project_commits()` and writes each commit file as its page arrives, keeping only the tip's tree; `av log --all` loads commit metadata without `tree` and retains only `--limit` entries while scanning (`heapq.nlargest`). Server side, GC marks reachability over streamed column tuples (`av_server/gc_mark.py`, `yield_per`), never ORM instances; audit export is a `StreamingResponse` over a server-side cursor in its own session; every list endpoint has a `limit` maximum and the formerly whole-table ones page at 500 by default (`tests/test_server_units.py` walks every GET route and fails on a new unbounded `limit`); `/api/refs` pages at 1000 (max 5000, ordered by name) — `VaultClient.list_refs` and `registry export` walk `offset` until a short page, so callers still get the complete mapping.
...
Every working-tree write funnels through one function: `python/av_cli/core.py::_materialize_tree()`. Checkout, clone, pull, and merge all call it; none of them maintain a second materialization path. It writes the target tree's files (downloading missing objects from the registry when only the remote has them), removes files present in the old tree but absent from the target one, and re-stats after materializing so the index reflects fresh mtimes — meaning `av status` reads clean right after a switch instead of reporting phantom modifications.
...
| `av stash pop`/`apply` | The stash record's own per-file `hash`/`layers`/`chunks` (via `materialize_file()` directly, not the whole-tree helper — see `cmd_history.py`) |
| `av fetch` | Not a caller — populates `.av/objects` only, via `sync.download_selected_objects()`, deliberately never touching the working tree |
...
**V1.6.0 — no double-store for split artifacts (Probleme.md #155).** `materialize_file()` used to reassemble a layer-split or CDC-chunked entry into the working tree, then *also* `shutil.copy2` the reassembled bytes into `.av/objects/<whole-hash>` — duplicating the full artifact on disk on every single restore, the exact inverse of what splitting exists to save. That second write is gone; a split entry reassembles into the working tree and stops there. Trade-off, accepted: a repeat checkout of the same split entry now always re-reassembles from shards rather than short-circuiting off a cached whole blob. Whole-file entries still `shutil.copy2` from `.av/objects` — Python's `shutil` has no reflink API and `os.link` is ruled out (a writable working-tree file must never alias the CAS).

**V1.6.0 — batch prefetch before reassembly.** The flat-tree branch of `_materialize_tree()` now calls `sync.ensure_objects_local(repo_root, client, tree)` once, up front, before its per-file loop — one batch-check round trip covering every layer/chunk/whole-file hash the whole tree references, then parallel downloads of only what's genuinely missing (sized by `python_pool_size(resolve_threads(...))`, not a hard-coded worker count). This mostly supersedes the older per-shard mitigation below for `checkout`/`stash`/`merge` (clone/pull already called `ensure_objects_local` themselves): by the time the per-file loop runs, the shards it needs are already local in the common case, so `materialize_file()`'s own per-part `server_available()` + serial `download_object` fallback rarely fires at all — it stays in place as the last-resort path (e.g. an object the prefetch batch-check somehow missed).

`materialize_file()` calls `client.server_available()` once per missing layer/chunk shard when reassembling a split artifact — for a checkpoint with hundreds of shards this used to be hundreds of real 2s-timeout-capable HTTP round trips against the same server, on the same `VaultClient` instance, in a tight loop. V1.5.0's `server_available()` 1-second per-instance cache (see the Commit Contract) collapses that to effectively one real probe per reassembly without any change to `materialize_file()` itself — this was the direct cause of the "fetch whole checkpoint" BAD benchmark row. V1.6.0's batch prefetch above goes further: in the common case this per-shard path is never reached at all.
