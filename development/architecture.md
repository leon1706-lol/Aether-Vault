...
## Daemon Contract (V1.5.0)

`av daemon` is an opt-in, off-by-default background process that executes `add`/`status`/`commit` — chosen because they run often enough for Python's own interpreter/import startup (~450-600ms even after the V1.5.0 import-graph fix) to matter — without paying that cost per invocation. It is additive, not a second implementation: `python/av_cli/daemon.py::DaemonServer._execute()` calls `cli.main(args=argv, standalone_mode=False, ...)`, the exact same click command objects the in-process path uses, with `sys.stdout`/`stderr` redirected into buffers and `os.chdir`/an allowlisted env overlay applied for the request's duration. This is what makes a byte-identical-output test against the in-process path possible, and it means the daemon can only ever make a command faster, never behave differently.

**Entry point restructure.** `[project.scripts] av` points at `python/av_cli/launcher.py:main`, not `av_cli.main:run` — `launcher.py` imports only `os`/`sys` at module scope and decides daemon-vs-fallback before touching `click`/`core.py` at all; any fallback (including any exception on the daemon path) does `from .main import run; run()`. `av_cli.main:run` is untouched and still directly invocable.
...
**Lifecycle.** Three-state enablement (`daemon_common.enabled_mode()`), checked in this precedence order: `AV_NO_DAEMON=1` always wins; `AV_DAEMON=1` or `.av/config`'s `"daemon": {"enabled": true}` opts into auto-spawn (a cold repo silently fires one off in the background via `daemon_client.maybe_auto_spawn()` — guarded by the same `O_CREAT|O_EXCL` lock file `av daemon start` uses, so two concurrent invocations never double-spawn — while the *current* invocation still always runs in-process rather than waiting on a cold daemon's warm-up); otherwise the default is to use an already-running daemon if one exists but never spawn one — `av daemon start` itself is the opt-in in that mode. Idle timeout defaults to 900s (`--idle-timeout` overrides); the daemon also exits on `.av` disappearing or 3 consecutive internal errors, so a poisoned daemon cannot serve garbage indefinitely.

**Detached spawning on Windows** tries `CREATE_BREAKAWAY_FROM_JOB` first (pip/distlib's `av.exe` console-script stub commonly wraps its own child in a Job Object with kill-on-close semantics, which a spawned daemon needs to escape), falling back to plain `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP` on `PermissionError` (a job that itself disallows breakaway). See Probleme.md #144 for a documented, verified environment limitation this cannot code around: some sandboxed dev/CI environments tear down *any* detached child (even one launched via Windows Task Scheduler, which normally escapes any job object) on a fixed timeline — `av daemon start --foreground` under an external supervisor is the correct pattern there.

| Situation | Behavior |
|---|---|
| No daemon running, mode = default | In-process, unchanged |
| No daemon running, mode = auto-spawn | In-process for this call; a daemon is fired off in the background for next time |
