...
**Idle trim (V1.6.1).** A separate, shorter threshold than the idle-timeout shutdown above: `AV_DAEMON_TRIM_SECS` (default 30) of inactivity triggers `DaemonServer.maybe_trim_idle()` on the existing 2s should-stop watchdog tick — `aether_core.release_pool()` (frees the C++ thread pool), `gc.collect()`, and (glibc Linux only; a no-op elsewhere) `ctypes`-called `malloc_trim(0)` to hand freed heap pages back to the OS. Runs at most once per idle stretch (armed again only by the next served request) and never contends with a real command — a non-blocking `_exec_lock` acquire means a trim that can't get the lock this tick just retries on the next one. `av daemon status` surfaces `rss_mb` (Linux: `/proc/self/statm`; Windows: `GetProcessMemoryInfo` via `ctypes`, `wintypes.HANDLE` argtypes required — see Probleme.md #170 for why; macOS: `resource.getrusage`), `uptime_s`, `requests_served`, and `trimmed` (whether at least one trim has happened), all refreshed after every served request and every trim — the state file is a point-in-time snapshot otherwise, previously only written once at startup.

**Memory envelope (V1.6.1).** Measured on the reference dev box (idle, no repo activity, no `AV_STAGE_FUSED`/hashing work in flight): a freshly auto-spawned daemon sits around 15-18 MB RSS before its first trim, dropping by roughly 1 MB after `maybe_trim_idle()` runs (`gc.collect()` reclaiming interpreter garbage; this box's `aether_core` pool and glibc `malloc_trim` don't apply on Windows). These are real measured numbers from this box, not the original plan's pre-implementation estimate (60-90 MB) — expect the number to scale with whatever a repo's own commands actually allocated before the daemon went idle (a large `add`/`commit` leaves more for `release_pool()`/`gc.collect()` to reclaim than a bare `status`). Staging peak (`AV_STAGE_FUSED`'s ring buffers, WS4.1) stays ≤ 384 MB at the default 8 workers regardless of daemon idle-trim — `AV_STAGE_BUFFER_MB`/`AV_THREADS` lower it, and it is orthogonal to the idle-trim mechanism above (staging memory is freed when the staging call returns, trim or no trim).
