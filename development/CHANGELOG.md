...
- **Attempted fix, reverted after breaking something worse**: `tests.yml`'s `on: push` had no tag filter, so pushing/moving a release tag re-triggered the entire test matrix against an already-tested commit, racing the gate a second way. Added `tags-ignore: ['**']` to stop that — but on the very next real commit this suppressed the branch-push trigger entirely (9 check-runs registered instead of the usual ~40; `security`/`codeql` ran, all ~23 of `tests.yml`'s own jobs silently didn't), confirmed directly against the checks API. Reverted to the plain, previously-proven `on: push:` with no filter; the tag-push race itself is unaffected — the pagination fix above already makes the gate correct once that redundant run finishes, it just costs one extra wait cycle after tagging instead of zero.
- **Verified**: `tests/test_release_gate.py`'s existing 27 tests pass unchanged; `test (3.14)` and every other required job green on the carrying commit after the revert; `git status` confirmed only the intended files touched throughout. Shipping this as `v1.3.7` rather than continuing to re-tag `v1.3.6`, since none of it changes product behavior — it's entirely the release pipeline's own bookkeeping catching up to itself.
