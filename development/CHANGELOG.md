...
- **Related fix, same investigation**: `tests.yml`'s `on: push` had no tag filter, so pushing/moving a release tag re-triggered the entire test matrix against a commit that had already been tested on its branch push — racing the gate a second, different way (it always reads the *most recent* check-run per name). Added `tags-ignore: ['**']` so a tag push no longer spawns a redundant run.
- **Verified**: `tests/test_release_gate.py`'s existing 27 tests pass unchanged; `test (3.14)` and every other required job green on the carrying commit; `git status` confirmed only the intended files touched throughout. Shipping this as `v1.3.7` rather than continuing to re-tag `v1.3.6`, since none of it changes product behavior — it's entirely the release pipeline's own bookkeeping catching up to itself.
