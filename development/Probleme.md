...
### 184. `verify_audit_chain`'s streamed DB cursor was never closed on its early-return path — the connection pool could hang forever on shutdown

**Severity:** 8/10 (a full `server-tests-windows` CI run hung silently for 20+ minutes at session teardown, twice in a row, after every one of its 196 tests had already passed -- indistinguishable from a wedged runner until diagnosed) · **Status:** 🟢 `fixed` (2026-09-14, V1.6.3).

**Problem:** `GET /api/admin/audit/verify` (`verify_audit_chain`, V1.6.3's streaming rewrite) opens `db.stream(stmt.execution_options(yield_per=500))` and iterates it with `async for row in result.scalars()`. On a broken chain the function `return`s as soon as the mismatch is found -- before the stream is exhausted -- leaving an asyncpg connection with an open, undrained server-side portal checked back into the pool when the request's session closes. `tests/test_server.py::TestAuditChain::test_verify_detects_a_tampered_row` hits exactly this path. The poisoned connection caused no visible failure for the rest of the run; only the session-scoped `client` fixture's teardown (`TestClient.__exit__` → the FastAPI lifespan's shutdown → `engine.dispose()`, which must cleanly close every pooled connection) blocked indefinitely trying to close it -- the very last thing the whole test file does, so every one of its 196 tests reported passing first.

**How it was found:** `server-tests-windows` was cancelled by its timeout twice during this phase, both times with every test already `PASSED` in the log and a multi-hundred-second gap before the cancellation with no further output. Reproduced locally: a full single-process `pytest tests/test_server.py` run hung identically after its last test (confirmed genuinely stuck, not slow, via `Get-Process`'s CPU counter showing a 0ms delta over repeated 8-10s windows). A targeted re-run of just the tampered-chain test plus the file's later tests, with the fix applied, exited in under 10 seconds after its last test passed -- versus never exiting at all before.

**Fix:** the iteration is wrapped in `try/finally`, calling `await result.close()` unconditionally so the early-return path releases the streaming cursor (and the connection's portal) exactly like the normal exhaustion path does.

**Verification:** the exact repro above, before and after -- pre-fix: full-suite run hangs indefinitely at teardown (reproduced, then force-killed after confirming zero CPU activity); post-fix: targeted re-run covering the same trigger test exits cleanly, `12 passed ... in 75.92s`, `PYTEST_EXIT=0`. `tests/test_server.py::TestFootprintV163::test_audit_verify_walks_the_whole_chain_via_a_cursor` (the non-early-return path) stays green, confirming the fix didn't touch normal-completion behavior.

### 185. `os.replace()` was assumed atomic-and-silent for two threads racing the same CAS object on Windows — it can raise `PermissionError` instead

**Severity:** 7/10 (a correctness/reliability bug in the parallel staging path used by every `av add` of 2+ files: two files with identical content -- a real, common case for ML checkpoints -- could crash `av add` outright on Windows if their staging happened to race) · **Status:** 🟢 `fixed` (2026-09-14, V1.6.3).

**Problem:** Three call sites (`_atomic_publish_object`, and both branches of `hash_and_publish_whole_file` -- the native `aether_core` path and the pure-Python fallback) publish a content-addressed object by writing to a uniquely-named temp file, then `os.replace(tmp, obj_path)`. All three carried the same comment/assumption: "os.replace still lands atomically" when two workers race the identical destination (same content hash). That is true on POSIX (rename-onto-existing is atomic; the loser's rename silently succeeds with byte-identical content) but **not proven true on Windows**, where the underlying `MoveFileExW` call can raise `PermissionError` ([WinError 5], ERROR_ACCESS_DENIED) when two renames target the same destination path concurrently -- found live running `av benchmark --lowmem`'s `noop_status_speed` benchmark, whose fixture stages several files, two of which hash to the same object; the parallel staging path (`cmd_staging.py`'s `ThreadPoolExecutor`) raced them and crashed the whole `av add` with an uncaught `PermissionError`.
