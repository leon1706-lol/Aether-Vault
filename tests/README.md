...
~330 tests across 20 files covering the CLI, the C++ bindings, the registry server, the
...
| `test_sync.py` | `av clone`/`av pull` against a fake in-process registry built on the real `VaultClient` |
...
| `test_server.py` | FastAPI suite against live Postgres+Redis (skips when unreachable), incl. merge-parent round-trip and the two-repo clone/pull E2E |
